...
typedef benchmark_result_t (*benchmark_function_t)(
    const uint64_t *, size_t, const kssd_array_t *, const uint64_t *);
...
/*
 * Preserve the historical reader semantics: only the first FASTA record is
 * used, and non-ACGT symbols inside that record are skipped without resetting
 * the rolling encoding. This behavior is documented as a migration caveat.
 */
static int read_first_fasta_record(const char *path, encoded_input_t *input)
{
    ...
    if (file == NULL) {
        fprintf(stderr, "cannot open FASTA input %s: %s\n",
                path, strerror(errno));
        return 0;
    }
    ...
    if (ferror(file) != 0) {
        fprintf(stderr, "cannot read FASTA input %s\n", path);
        fclose(file);
        free(input->values);
        memset(input, 0, sizeof(*input));
        return 0;
...
/* Exact x64_128 MurmurHash3 result word for the benchmark's 8-byte input. */
...
{
    uint32_t words[2];
    uint64_t seed = wy_mix(secret[0], secret[1]);
    uint64_t left;
    uint64_t right;
    ...
    right ^= seed;
    wy_multiply(&left, &right);
    return wy_mix(left ^ secret[0] ^ UINT64_C(8), right ^ secret[1]);
}

#define DEFINE_MINIMIZER_BENCHMARK(function_name, hash_expression)             \
    ...
    {                                                                           \
        ...
        for (offset = 0U; offset < (size_t)W; ++offset) {                       \
            const uint64_t kmer = kmers[offset];                                \
        ...
        for (window_start = 1U; window_start < window_count; ++window_start) {  \
            const size_t previous_position = window_start - 1U;                 \
            const size_t new_position = window_start + (size_t)W - 1U;         \
        ...
        result.minimizer_count = window_count;                                  \
        result.checksum = current_minimum ^ (uint64_t)current_position;         \
        benchmark_sink ^= result.checksum;                                      \
        return result;                                                          \
    }

DEFINE_MINIMIZER_BENCHMARK(
                              ...
                              size_t *checked)
{
    ...
    for (index = 0U; index < sample_count; ++index) {
        ...
        const uint64_t context_value =
            kssd_array_map_unchecked(context, encoded);
        const uint64_t fast_value =
            kssd_array_fast_with_tables(encoded, context);
        if (context_value != fast_value) {
            fprintf(stderr,
                    "fast/context parity failed at sample %zu: context=%" PRIu64
                    " fast=%" PRIu64 "\n",
                    index, context_value, fast_value);
            return 0;
        }
    }
...
int main(int argc, char **argv)
{
    ...
    if (argc != 4) {
        fprintf(stderr, "usage: %s FASTA REPEAT SEED\n", argv[0]);
        return EXIT_FAILURE;
    }
    fasta_path = argv[1];
    if (!parse_uint64(argv[2], &repeat) || repeat == UINT64_C(0) ||
        !parse_uint64(argv[3], &seed)) {
        fputs("REPEAT must be positive and SEED must be an integer\n", stderr);
        return EXIT_FAILURE;
    }
    if (!read_first_fasta_record(fasta_path, &input)) {
        return EXIT_FAILURE;
    }
    if (input.count < (size_t)W) {
        fprintf(stderr,
                "input has %zu encoded k-mers but W=%d requires at least %d\n",
                input.count, W, W);
        free(input.values);
        return EXIT_FAILURE;
    }
    ...
    for (method = 0U;
        ...
        const benchmark_result_t result =
            benchmark_functions[method](input.values,
                                        input.count,
                                        &context,
                                        wy_secret);
