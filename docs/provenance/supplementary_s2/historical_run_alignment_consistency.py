...
INTEGRATION_DIR = SCRIPT_DIR.parents[1]
DEFAULT_CONFIG = REPO_ROOT / "configs/minimap2_supplementary_alignment.json"
...
def resolve_path(relative: str, override: Path | None) -> Path:
    ...
    candidates.append(REPO_ROOT / "data/external" / relative)
    candidates.append(REPO_ROOT / "data/external" / Path(relative).name)
...
def executable_paths(phase5b: Path, config: dict[str, object]) -> dict[str, Path]:
    paths = {
...
def main() -> int:
    ...
    executables = executable_paths(phase5b, config)
    if args.preflight:
        datasets: list[dict[str, object]] = []
